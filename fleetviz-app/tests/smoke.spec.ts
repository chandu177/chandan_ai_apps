...
test('smoke test - delivery map dashboard loads at home', async ({ page }) => {
  await page.goto('/');

  await expect(page.getByRole('heading', { name: APP_CONFIG.name })).toBeVisible();
  await expect(page.getByRole('heading', { name: 'Delivery map' })).toBeVisible();
  await expect(page.getByText('Delivery tracking dashboard')).toBeVisible();
  await expect(
    page.getByLabel('Delivery time scrubber').or(page.getByText(/No location events found/i)),
